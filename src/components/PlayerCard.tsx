    ...
    >
      <div className="flex items-center justify-between">
        <div>
          ...
          <div className="text-sm text-gray-500 mt-0.5">
            ...
            {player.pendingBuyIn > 0 && (
              <span className="text-yellow-600"> (+£{player.pendingBuyIn} pending)</span>
            )}
          </div>
        </div>
        <div className="text-right">
          {hasCashedOut ? (
            <div>
              <div className="text-sm text-gray-500">Cashed out</div>
              <div className={`font-bold ${player.net! >= 0 ? "text-emerald-600" : "text-red-500"}`}>
                {formatNet(player.net!)}
              </div>
            </div>
          ) : (
            <div className="text-gray-400 text-2xl">&rsaquo;</div>
          )}
        </div>
      </div>
    </button>
